...
arithmetic and is comfortable with [modulo operations](https://en.wikipedia.org/wiki/Modulo_operation),
...
case that $n$ is a prime number, the computation of $\phi(n)$ is easy, all
...
the input and output domain, which si the result that helps prove the following statements:
$$\begin{aligned}
...
will be the [Hadstadt attack](./2020-01-06-hadstadt.html).
